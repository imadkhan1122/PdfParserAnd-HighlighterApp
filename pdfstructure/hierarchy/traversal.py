...
def get_elements_by_page(document, current_page = 0):
    dic = {'width':0, 'hight':0, 'elements':[]}
    for  e in traverse_in_order(document):
        if e.page_number == current_page:
            dic['width'] = e.page_w
            dic['height'] = e.page_h
            dic['page_number'] = e.page_number
            dic['elements'].append({'style':e.span_styles, 'block':e.block_styles,'text':e.text, 'level':e.level, 'label':e.label})
    return dic