...
def condition_boldness(h1: Section, h2: Section):
    ...
    @return:
    """
    h1start = next(word_generator(h1))
    h2start = next(word_generator(h2))
    if numeration_pattern.match(h2start) and not numeration_pattern.match(h1start):
        return False
    return h1.span_styles['font_stats']['bold'] and not h2.span_styles['font_stats']['bold']
...
def condition_h1_enum_h2_not(h1: Section, h2: Section):
    ...
    """
    if h2.span_styles['font_stats']['bold']  and not h1.span_styles['font_stats']['bold'] :
        return False
...
def condition_h1_slightly_bigger_h2(h1: Section, h2: Section):
    ...
    @return:
    """
    return h1.span_styles['font_stats']['size'] == h2.span_styles['font_stats']['size'] \
           and h1.span_styles['font_stats']['size'] - h2.span_styles['font_stats']['size'] > 1.0
